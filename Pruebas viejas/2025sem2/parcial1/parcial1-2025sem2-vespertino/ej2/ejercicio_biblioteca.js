...
function devolverLibro(idLibro, idEstudiante, fechaDevolucion) {
  ...
  if(!estudiante.librosActuales.includes(idLibro)){
    return false;
  }
  const prestamo = libro.prestamos.find(prestamo => prestamo.estudiante === estudiante.nombre && prestamo.fechaDevolucion === null);
  if(!prestamo){
    return false;
  }
  prestamo.fechaDevolucion = fechaDevolucion
  libro.disponible = true;
  estudiante.librosActuales = estudiante.librosActuales.filter (id => id !== idLibro);
  return true
...
function listarLibrosPrestados(incluirDetallesEstudiante = false) {
  const librosPrestado = [];
  for(const libro of biblioteca.libros){
    if(!libro.disponible){
      if(incluirDetallesEstudiante){
        const prestamo = libro.prestamos.find(prestamo => prestamo.fechaDevolucion === null);
        const estudiante = biblioteca.estudiantes.find(estudiante => estudiante.nombre === prestamo.estudiante)
        librosPrestado.push({...libro,
